...
<?php
	if (isset($submit))
	{
		...
		{
			$sql = "delete from tbl_vehicle_guide where guide_id=$array[$x]";
			// print $sql;
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	<table width="98%" border="0" cellspacing="0" cellpadding="2" align="center">
       <tr>
         <td class="pophead">Delete GUIDE</td>
       </tr>         
       <tr>
		 ...
		 <?php
			...
			$total_sql = "select * from tbl_vehicle_guide";
    ...
    <tr>
		<td>
			...
			<?php
				$faq_sql = "select * from tbl_vehicle_guide limit $start_record,$num_per_page";
				...
				while($row = mysql_fetch_array($faq_result))
				{
					...
					print '<td valign=top width=15><strong>'.$i.'</strong></td>';
					print '<td>'.$row[query].'</td><td>';
					if ($row[approve] == 'yes')
						print "<img src=\"../pictures/approved.jpg\">";
					else
						print "<img src=\"../pictures/napproved.jpg\">";
					print '</td><td>'.$row[post_date].'</td>';
					print '<td valign=top><input type=checkbox name=array[] value='.$row[guide_id].'></td>';
					print '</tr>';
					$i++;
				}
			?>
