...
<?php
	if (isset($submit))
	{
		...
		{
			$p_sql = "select prod_key from tbl_product where product_id=$array[$x]";
			$p_result = mysql_query($p_sql) or die(mysql_error());
			$p_row = mysql_fetch_array($p_result);
			$prod_key = $p_row['prod_key'];
			
			$sql = "delete from tbl_product where product_id=$array[$x]";
			// print $sql;
			...
			{
				unlink("../library/thumbs/".$prod_key.'.jpg');
				unlink("../library/enlarge/".$prod_key.'.jpg');
				
				$today = date("Y-m-d");
				$update_sql = "update tbl_last_updated set date='$today' where id=1";
				$update_result = mysql_query($update_sql);
				
				print "<font color=#FF0000><p>Product ".$array[$x]." has been deleted successfully.<br>";
			}
			$x++;
		}
	}
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	<table width="98%" border="0" cellspacing="0" cellpadding="2" align="center">
       <tr>
         <td class="linktext"><strong>Delete Products</strong></td>
       </tr>         
       <tr>
		 ...
		 <?php
			...
			$total_sql = "select * from tbl_product";
    ...
    <tr>
		<td class="smallblack">
            ...
            <form action="<?php print $_SERVER['PHP_SELF']; ?>" method="post" name="deleteFAQ" id="deleteFAQ">
			<?php
				$faq_sql = "select * from tbl_product limit $start_record,$num_per_page";
				...
				while($row = mysql_fetch_array($faq_result))
				{
					...
					print '<td class=smallblack valign=top>'.$row[prod_key].'</td>';
					print '<td class=smallblack valign=top>'.$row[product_name].'</td>';
					print '<td width=10><input type=checkbox name=array[] value='.$row[product_id].'></td>';
					print '</tr>';
					$i++;
				}
			?>
