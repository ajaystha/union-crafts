...
function editor_generate(objname,userConfig) {
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      toolbar += '<button title="' +btnTitle+ '" id="_' +objname+ '_' +btnCmdID+ '" class="btn" onClick="' +btnOnClick+ '" onmouseover="if(this.className==\'btn\'){this.className=\'btnOver\'}" onmouseout="if(this.className==\'btnOver\'){this.className=\'btn\'}" unselectable="on"><img src="../cms/%27%20+config.imgURL%20+%20btnImage+%20%27" border=0 unselectable="on"></button>';
...
function editor_action(button_id) {
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
      ...
      showModalDialog(_editor_url + "popups/insert_news_image.htm", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }
 ...
 // src="' +_editor_url+ 'popups/blank.html"
  ...
  //
    ...
    if (config.stylesheet) {
      html += '<link href="../cms/%27%20+config.stylesheet+%20%27" rel="stylesheet" type="text/css">\n';
    }
    html += '<style>\n';
