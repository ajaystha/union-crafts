...
<?php
	if (isset($submit))
	{
		...
		{
			$sql = "delete from tbl_news where news_id=$array[$x]";
			// print $sql;
			...
			else
			{
				...
				print "<font color=#FF0000><p>NEWS ".$array[$x]." has been deleted successfully.<br>";
			}
			$x++;
		}
	}
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	<table width="98%" border="0" cellspacing="0" cellpadding="2" align="center">
       <tr>
         <td class="linktext"><strong>Delete NEWS</strong></td>
       </tr>         
       <tr>
		 ...
		 <?php
			...
			$total_sql = "select * from tbl_news";
    ...
    <tr>
		<td>
			...
			<?php
				$faq_sql = "select * from tbl_news limit $start_record,$num_per_page";
				$faq_result = mysql_query($faq_sql) or die(mysql_error());
				$i = 1;
				while($row = mysql_fetch_array($faq_result))
				{
					print '<tr>';
					print '<td class=smallblack width=12 valign=top>'.$i.'</td>';
					print '<td class=smallblack>'.substr($row[news_in_detail],0,100).'</td>';				
					print '<td><input type=checkbox name=array[] value='.$row[news_id].'></td>';
					print '</tr>';
					$i++;
				}
			?>
